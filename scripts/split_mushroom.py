#!/usr/bin/env python3
"""
Create a random train / test / pool split for Mushroom active learning.

Usage (from repo root):
    .venv/bin/python scripts/split_mushroom.py
    .venv/bin/python scripts/split_mushroom.py --seed 0

Reads preprocessed targets from data/mushroom/targets.parquet (or .csv).
Writes:
    data/mushroom/split.json
    data/mushroom/split_report.json
"""
...
def _load_targets(data_dir: Path):
    ...
    raise FileNotFoundError(
        f"No targets file in {data_dir}. Run scripts/preprocess_mushroom.py first."
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Random Mushroom train/test/pool split")
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=DEFAULT_DATA_DIR,
        help="Directory with preprocessed Mushroom files",
    )
    parser.add_argument(
        "--seed",
        type=int,
