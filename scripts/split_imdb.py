#!/usr/bin/env python3
"""
Create a random train / test / pool split for IMDB active learning.

Usage (from repo root):
    .venv/bin/python scripts/split_imdb.py
    .venv/bin/python scripts/split_imdb.py --seed 0

Reads preprocessed targets from data/imdb/targets.parquet (or .csv).
Writes:
    data/imdb/split.json
    data/imdb/split_report.json
"""
...
def _load_targets(data_dir: Path):
    ...
    raise FileNotFoundError(
        f"No targets file in {data_dir}. Run scripts/preprocess_imdb.py first."
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Random IMDB train/test/pool split")
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=DEFAULT_DATA_DIR,
        help="Directory with preprocessed IMDB files",
    )
    parser.add_argument(
        "--seed",
        type=int,
