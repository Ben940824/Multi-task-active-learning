"""Telco Churn preprocessing (report_multitaskAL.pdf Table 1)."""
...
PAPER_ROW_COUNT = 7021
PAPER_TOTAL_COLUMN_COUNT = 20
PAPER_FEATURE_COUNT = 17

IDENTIFIER_COLUMNS = ["customerID"]
RAW_TARGET_COLUMNS = ["Churn", "Contract", "PaymentMethod"]
TARGET_COLUMNS = ["target_churn", "target_contract", "target_payment"]
RAW_TO_TARGET = {
    "Churn": "target_churn",
    "Contract": "target_contract",
    "PaymentMethod": "target_payment",
}
...
PAPER_IR: dict[str, tuple[float, float]] = {
    "target_churn": (2.781, 2.781),
    "target_contract": (2.444, 2.616),
    "target_payment": (1.519, 1.55),
}
...
class PreprocessConfig:
    ...
    notes: list[str] = field(
        default_factory=lambda: [
            ...
            "Paper row count matches drop_duplicates without dropping blank TotalCharges.",
            "TotalCharges coerced to numeric; blank strings become NaN but rows kept.",
        ]
    )
...
def preprocess_telco(raw_path: Path | str) -> PreprocessResult:
    ...
    # Coerce TotalCharges; paper instance count aligns with keeping blank rows.
...
def print_validation_summary(result: PreprocessResult) -> None:
    print("=" * 60)
    print("Telco Churn preprocessing validation vs paper Table 1")
    print("=" * 60)
